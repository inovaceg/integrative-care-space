...
export function ServiceGrid({ services, tone }: { services: Service[]; tone: "psychology" | "biomedicine" }) {
  ...
  return <div className="card-grid">{services.map((service) => { const Icon = service.icon; return <article key={service.title} className={`service-card ${toneClass}`}><div className="card-topline"><span className="service-icon"><Icon /></span><span className="card-index" aria-hidden="true">+</span></div><h2 className="card-title mt-7">{service.title}</h2><p className="mt-4 min-h-20 text-sm leading-6 text-muted-foreground">{service.description}</p><a href={whatsappUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-primary">Saiba mais <ArrowRight className="size-4" /></a></article>; })}</div>;
}
