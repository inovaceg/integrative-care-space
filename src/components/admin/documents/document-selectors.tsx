...
export function ProfessionalAreaSelector({ value, onChange, onContinue }: { value: ProfessionalArea | null; onChange: (value: ProfessionalArea) => void; onContinue: () => void }) {
  return <section className="space-y-6 rounded-xl border border-slate-200 bg-white p-6"><h3 className="font-display text-xl">Em qual área profissional o documento será emitido?</h3><div className="grid gap-4 md:grid-cols-2">{(Object.keys(professionalAreas) as ProfessionalArea[]).map(key => {
    const area = professionalAreas[key]; const Icon = key === 'psicologia' ? Brain : Stethoscope;
    return <button key={key} type="button" aria-pressed={value === key} onClick={() => onChange(key)} className={`rounded-xl border-2 p-6 text-left transition-colors ${value === key ? 'border-[#2f8f82] bg-[#eff9f6]' : 'border-slate-200 hover:border-[#2f8f82]'}`}><Icon className="mb-4 size-7 text-[#2f8f82]" /><strong className="block text-lg uppercase">{area.label}</strong><p className="mt-4 text-sm">{professionalName}</p><p className="mt-2 text-sm text-slate-600">{area.title}</p><p className="mt-1 font-semibold text-[#21655f]">{area.registration}</p></button>;
  })}</div><Button disabled={!value} onClick={onContinue}>Continuar</Button></section>;
}
