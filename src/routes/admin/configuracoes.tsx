...
function SaveButton({ saved, onSave }: { saved: boolean; onSave: () => void }) {
  return <div className="mt-6 flex items-center gap-3"><Button type="button" onClick={onSave} className="bg-[#2f8f82] text-white hover:bg-[#26796e]">Salvar alterações</Button>{saved && <span className="inline-flex items-center gap-1.5 text-xs text-[#28786e]"><Check className="size-3.5" /> Alterações salvas nesta sessão</span>}</div>;
}
