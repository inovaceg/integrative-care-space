...
function ReportsPage() {
  ...
  const revenueByMonth = useMemo<ReportMonth[]>(() => months.map((month) => ({
    ...month,
    value: appointments
      .filter((appointment) => appointment.status !== "cancelled" && Number(appointment.amount_paid ?? 0) > 0 && monthKey(new Date(appointment.start_time)) === month.key)
