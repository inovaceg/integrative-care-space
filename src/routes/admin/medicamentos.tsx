...
function MedicationDialog({ medication, open, onOpenChange, onSaved }: { medication: MedicationRecord | null; open: boolean; onOpenChange: (open: boolean) => void; onSaved: (medication: MedicationRecord) => void }) {
  const { user } = useAuth();
  const [form, setForm] = useState<MedicationInput>({ name: "", unit: "unit", supplier: null, notes: null });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    if (!open) return;
    setForm({ name: medication?.name ?? "", unit: medication?.unit ?? "unit", supplier: medication?.supplier ?? null, notes: medication?.notes ?? null });
    setError("");
  }, [medication, open]);
